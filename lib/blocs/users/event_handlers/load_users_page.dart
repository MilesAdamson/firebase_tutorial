...
FutureOr<void> loadUsersPage(
  UsersLoadPageEvent event,
  UsersState state,
  Emitter<UsersState> emit,
  UsersRepository repository,
) async {
  try {
    emit(state.copyWith(loadUsersPageProcess: Process.loading()));

    final incomingDocuments =
        await repository.getPageOfUsersAlphabetically(event.startAt);
    final userDocuments =
    ...
    emit(state.copyWith(
      ...
      loadUsersPageProcess: Process.success(
        result: incomingDocuments.map((doc) => doc.data()!).toList(),
      ),
    ));
  } catch (e, s) {
    final error = "$e\n$s";
    debugPrint(error);
    emit(state.copyWith(loadUsersPageProcess: Process.failed(error)));
  }
}
